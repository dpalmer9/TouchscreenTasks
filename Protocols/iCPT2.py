...
class Experiment_Staging(FloatLayout):
    def __init__(self,trial_max,session_max,block_max,block_count,stimulus_duration,limited_hold,target_prob,probe_check,id_entry,**kwargs):
        ...
        self.stimulus_duration_default = stimulus_duration
        self.limited_hold_default = limited_hold
        self.stimulus_duration = self.stimulus_duration_default
        self.limited_hold = self.limited_hold_default
        self.block_hold_time = 5
        self.end_hold_time = 5

        self.target_prob = target_prob
        self.distractor_prob = 1 - target_prob
        self.image_list = []
        self.correct_image_list = ['10','4']
        self.incorrect_image_list = ['1','2','3','5','9','11','13','15']
        self.total_image_list = self.correct_image_list + self.incorrect_image_list
        self.target_prob_indiv = self.target_prob / len(self.correct_image_list)
        self.distractor_prob_indiv = self.distractor_prob / len(self.incorrect_image_list)
        self.image_prob_list = list()
        for a in range(0,len(self.correct_image_list)):
        ...
        self.current_image, self.contrast_list[self.contrast_list_pos])
        ...
        if self.distractor_active == 1:
            if self.distractor_congruent == 0:
                if self.current_image in self.correct_image_list:
                    self.distractor_image = np.random.choice(a=self.incorrect_image_list,size=None)
                else:
                    self.distractor_image = np.random.choice(a=self.correct_image_list,size=None)
            elif self.distractor_congruent == 1:
                self.distractor_image = self.current_image
    ...
    def image_presentation(self,*args):
        if self.image_on_screen == False:
            ...
            if self.current_stage == 5 and self.distractor_active == 1:
                if sys.platform == 'linux'or sys.platform == 'darwin':
                    self.image_path_d1 = '%s/Images/%s.png' % (self.curr_dir, self.distractor_image)
                elif sys.platform == 'win32':
                    self.image_path_d1 = '%s\\Images\\%s.png' % (self.curr_dir, self.distractor_image)
                ...
                if sys.platform == 'linux'or sys.platform == 'darwin':
                    self.image_path_d2 = '%s/Images/%s.png' % (self.curr_dir, self.distractor_image)
                elif sys.platform == 'win32':
                    self.image_path_d2 = '%s\\Images\\%s.png' % (self.curr_dir, self.distractor_image)
        ...
        if ((self.current_time - self.image_pres_time) >= self.stimulus_duration) and self.stimulus_on_screen == True:
            ...
            self.image_path = '%s/Images/black.png' % (self.curr_dir)
            self.limited_hold_image = ImageButton(source=self.image_path, allow_stretch=True)
        ...
        if ((self.current_time - self.image_pres_time) >= (self.stimulus_duration + self.limited_hold)):
            ...
            if (self.current_image in self.correct_image_list) or (self.current_image == 'snowflake'):
                self.current_correct = 0
    ...
    def record_data(self):
        ...
        self.data_file.write("%s,%s,%s,%s,%s,%s,%s" % (self.current_trial,self.stage_label,self.current_block,self.current_image_label,self.current_correction,self.trial_contingency,self.lat))
        self.data_file.close()
    ...
    def set_new_trial_configuration(self,*args):
        ...
        self.current_image = np.random.choice(a=self.total_image_list,size=None,p=self.image_prob_list)
        self.current_image_label = self.current_image
        ...
        if self.current_stage == 4:
            self.contrast_list_pos = random.randint(0, 3)
            self.image_name_contrast = '%s%s' % (self.current_image, self.contrast_list[self.contrast_list_pos])
        ...
        if self.current_stage == 5:
            ...
            self.current_image_label = '%s-%s-%s' % (self.current_image,self.distractor_label,self.congruent_label)
...
class Experiment_App(App):
    def build(self):
        experiment = Experiment_Staging(trial_max=self.trial_maximum,session_max=self.session_maximum,block_max = self.block_max,block_count=self.block_count,
                                        stimulus_duration=self.stimulus_duration,limited_hold=self.limited_hold,
                                        target_prob=self.target_prob,probe_check = self.probe_check,id_entry=self.id_value)
        return experiment
    def set(self,trial_max,session_max,block_max,block_count,stimulus_duration,limited_hold,target_prob,probe_check,id_entry):
        self.trial_maximum = trial_max
        self.session_maximum = session_max
        self.block_max = block_max
        self.block_count = block_count
        self.stimulus_duration = stimulus_duration
        self.limited_hold = limited_hold
        self.target_prob = target_prob
